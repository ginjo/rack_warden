		...
		def self.included(base)
		    ...
		    # Newere remember_me routine
				Warden::Strategies.add(:remember_me) do
				  def valid?
				  	App.logger.debug "RW checking existence of remember_token cookie: #{env['rack.cookies']['rack_warden_remember_me']}"
				    env['rack.cookies']['rack_warden_remember_me']      
				  end
				
		      def authenticate!
		      	App.logger.debug "RW authenticating with rack_warden_remember_me token: #{env['rack.cookies']['rack_warden_remember_me']}"
				...
				Warden::Manager.after_authentication  do |user, auth, opts|
					App.logger.debug "RW after_authentication callback - remember_token cookie: #{auth.env['rack.cookies']['rack_warden_remember_me']}"
	      	if user.is_a?(User) && user.remember_token
				...
				Warden::Manager.before_logout do |user, auth, opts|
				 	App.logger.debug "RW before_logout callback - remember_token cookie: #{auth.env['rack.cookies']['rack_warden_remember_me']}"
		    ...
		    Warden::Strategies.add(:password) do
		      ...
		      def authenticate!
		      	...
		      	# User-class based authenticator.
